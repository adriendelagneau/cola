...
const PRODUCT_CONFIG: Record<string, ResponsiveConfig> = {
  XL: {
...
const Scene = ({ flavor }: SceneProps) => {
  ...
  useGSAP(() => {
    ...
    mm.add("(min-width: 1280px)", () => setup(PRODUCT_CONFIG.XL));

    mm.add("(min-width: 1024px) and (max-width: 1279px)", () =>
      setup(PRODUCT_CONFIG.LG)
    );

    mm.add("(min-width: 768px) and (max-width: 1023px)", () =>
      setup(PRODUCT_CONFIG.MD)
    );

    mm.add("(max-width: 767px)", () => setup(PRODUCT_CONFIG.SM));
