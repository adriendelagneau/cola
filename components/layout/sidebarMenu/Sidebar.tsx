  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center justify-center pt-7">
        <button onClick={closeMenu} className="absolute top-7 left-6 text-2xl">
          <XIcon size={32} />
        </button>

        <MenuLink href="/" className="font-cream-cake text-6xl capitalize">
          Breizh Cola
        </MenuLink>
      </div>

      {/* MAIN */}
      <div className="flex flex-col lg:flex-row flex-1 p-12 justify-center">
        ...
        {/* LINKS */}
        <nav className="flex flex-1 flex-col justify-center items-center lg:items-start gap-6">
          {items.map((item, index) => (
            <div key={item.href} className="overflow-hidden">
              <MenuLink
                href={item.href}
                onMouseEnter={() => setActiveCan(index)}
                className="menu-link font-cream-cake group block translate-y-full text-5xl sm:text-6xl lg:text-7xl xl:text-8xl"
              >
                Breizh{" "}
                <span
      ...
      <div className="overflow-hidden p-8">
        ...
        >
          ...
          <Link href="https://facebook.com/breizh-cola">
            <svg
              className="fill-primary h-10 w-10 cursor-pointer sm:h-12 sm:w-12 lg:w-14"
              viewBox="0 0 31.5 58"
            >
